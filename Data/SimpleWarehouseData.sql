-- Simple Warehouse Data Creation
-- This script creates basic warehouse data for testing

-- Clear existing data
DELETE FROM WarehouseCells WHERE WarehouseId IN (SELECT Id FROM Warehouses);
DELETE FROM WarehouseClusters WHERE WarehouseId IN (SELECT Id FROM Warehouses);
DELETE FROM WarehouseZones WHERE WarehouseId IN (SELECT Id FROM Warehouses);
DELETE FROM Warehouses;
DELETE FROM Products WHERE CompanyId IN (SELECT Id FROM Companies);
DELETE FROM ProductCategories WHERE CompanyId IN (SELECT Id FROM Companies);
...
-- Insert Warehouse
INSERT INTO Warehouses (Name, Description, Address, Capacity, Status, IsActive, CreatedAt) VALUES
('KHO A', 'Kho chính của công ty', '123 Đường ABC, Quận 1, TP.HCM', 100000, 'Active', 1, GETUTCDATE());
...
-- Insert Warehouse Cells (40 cells: 4 rows x 10 columns) - exactly like mock data
-- Row A (A01-A10)
...
PRINT 'Warehouse data created successfully!';
